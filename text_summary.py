...
class TextSummary():
    def __init__(self, model="gemma2-9b-it"):
        ...
        self.client = OpenAI(
            ...
            api_key=GROQ_API_KEY.strip(),  # Ensure no whitespace
        )
        self.model = model
    ...
    async def __call__(self, article):
        ...
        if not article.strip():
            raise ValueError("Empty article provided for summarization")
            
        try:
            logging.info(f"Sending request to Groq API with model: {self.model}")
            ...
            chat_completion = self.client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a skilled news editor who creates concise, informative summaries. Focus on key facts, maintain journalistic integrity, and avoid speculation."
                    },
                    {
                        "role": "user",
                        "content": """Please summarize this article in 2-3 clear, informative sentences. 
                        Include the most important facts and maintain the original tone.
                        
                        Article:
                        {article}
                        
                        Summary:""".format(article=article)
                    }
                ],
                model=self.model,
            ...
            logging.error(f"Error in text summary: {str(e)}")
